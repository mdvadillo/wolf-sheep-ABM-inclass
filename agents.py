...
class Lawnmower(RandomWalker):
    """
    A lawnmower that walks around and cuts grass.
    """
    ...
    def step(self):
        ...
        #x, y = self.pos
        #this_cell = self.model.grid.get_cell_list_contents([self.pos])
        #grass = [obj for obj in this_cell if isinstance(obj, GrassPatch)]
        #if len(grass) > 0:
        #    grass_to_cut = self.random.choice(grass)
        #    grass_to_cut.fully_grown = False

            # Cut the grass
            #self.model.grid.remove_agent(grass_to_cut)
            #self.model.schedule.remove(grass_to_cut)
